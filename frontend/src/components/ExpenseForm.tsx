...
interface Props {
  ...
  onCancel: () => void;
  defaultValues?: { participante: string; valor: number; local: string; forecast: boolean; budgetId: number };
  isLoading?: boolean;
}

export default function ExpenseForm({ budgets, onSubmit, onCancel, defaultValues, isLoading }: Props) {
  const { register, handleSubmit, formState: { errors } } = useForm<ExpenseInput>({
    defaultValues: defaultValues ?? { forecast: false },
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        />
        {errors.participante && <p className="text-red-500 text-xs mt-1">{errors.participante.message}</p>}
      </div>
      <div>
        ...
        <input
          type="number"
          step="0.01"
          {...register("valor", { required: "Obrigatório", valueAsNumber: true, min: { value: 0.01, message: "Deve ser positivo" } })}
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
          placeholder="1500.00"
        />
      ...
      <div>
        ...
        <select
          {...register("budgetId", { required: "Obrigatório", valueAsNumber: true })}
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
        >
        ...
        {errors.budgetId && <p className="text-red-500 text-xs mt-1">{errors.budgetId.message}</p>}
      </div>
      <div className="flex items-center gap-3">
        <label className="relative inline-flex items-center cursor-pointer">
          <input type="checkbox" {...register("forecast")} className="sr-only peer" />
        ...
        <span className="text-sm font-medium text-gray-700">Previsão (Forecast)</span>
      </div>
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" disabled={isLoading} className="px-4 py-2 text-sm font-medium text-white bg-brand-600 rounded-lg hover:bg-brand-700 disabled:opacity-50">
          {isLoading ? "Salvando..." : defaultValues ? "Atualizar" : "Adicionar"}
        </button>
      </div>
    </form>
  );
